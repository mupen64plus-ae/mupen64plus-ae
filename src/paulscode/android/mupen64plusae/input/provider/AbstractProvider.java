...
public abstract class AbstractProvider
{
     ...
     * @return The unique identifier of the hardware (API-specific).
     */
    @TargetApi( 16 )
    public static int getHardwareId( InputEvent event )
    {
        return getHardwareId( event.getDevice() );
    }
    
    /**
     * Obtains unique hardware id associated with an input device.
     * 
     * @param device The peripheral device descriptor.
     * @return The unique identifier of the hardware (API-specific).
     */
    @TargetApi( 16 )
    public static int getHardwareId( InputDevice device )
    {
        if( AppData.IS_JELLYBEAN )
        {
            // This value is persistent across reboots
            return device.getDescriptor().hashCode();
        }
