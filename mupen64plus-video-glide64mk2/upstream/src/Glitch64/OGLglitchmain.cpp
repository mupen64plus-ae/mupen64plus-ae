                                         ...
                                         FxU32 				evenOdd)
{
  ...
  if (!use_fbo) {
    ...
    //create new FBO
    fb newFb;
    glGenFramebuffersEXT( 1, &(newFb.fbid) );
    glGenRenderbuffersEXT( 1, &(newFb.zbid) );
    glBindRenderbufferEXT( GL_RENDERBUFFER_EXT, newFb.zbid );
    glRenderbufferStorageEXT( GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT, width, height);
    newFb.address = pBufferAddress;
    newFb.width = width;
    newFb.height = height;
    newFb.texid = pBufferAddress;
    newFb.buff_clear = 0;
    add_tex(newFb.texid);
    glBindTexture(GL_TEXTURE_2D, newFb.texid);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0,
      GL_RGB, GL_UNSIGNED_BYTE, NULL);
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );

    glBindFramebufferEXT( GL_FRAMEBUFFER_EXT, newFb.fbid);
    glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
      GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, newFb.texid, 0);
    glFramebufferRenderbufferEXT( GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, newFb.zbid );
    glViewport(0,0,width,height);
    glScissor(0,0,width,height);
