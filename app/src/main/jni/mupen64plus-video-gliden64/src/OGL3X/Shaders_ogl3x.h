...
#endif // GL_USE_UNIFORMBLOCK
"void calc_light(in lowp float fLights, in lowp vec3 input_color, out lowp vec3 output_color) {\n"
"  lowp float nLights = floor(fLights + 0.5);	\n"
"  mediump vec3 n = normalize(input_color);	\n"
"  output_color = input_color*step(nLights, 0.1) + uLightColor[int(nLights)]*step(1.0, nLights);	\n"
"  output_color += max(dot(n, uLightDirection[0]), 0.0)*uLightColor[0]*step(1.0, nLights);	\n"
"  output_color += max(dot(n, uLightDirection[1]), 0.0)*uLightColor[1]*step(2.0, nLights);	\n"
"  output_color += max(dot(n, uLightDirection[2]), 0.0)*uLightColor[2]*step(3.0, nLights);	\n"
"  output_color += max(dot(n, uLightDirection[3]), 0.0)*uLightColor[3]*step(4.0, nLights);	\n"
"  output_color += max(dot(n, uLightDirection[4]), 0.0)*uLightColor[4]*step(5.0, nLights);	\n"
"  output_color += max(dot(n, uLightDirection[5]), 0.0)*uLightColor[5]*step(6.0, nLights);	\n"
"  output_color += max(dot(n, uLightDirection[6]), 0.0)*uLightColor[6]*step(7.0, nLights);	\n"
