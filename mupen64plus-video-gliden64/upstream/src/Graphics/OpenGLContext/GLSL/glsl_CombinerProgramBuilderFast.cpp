...
public:
	...
	void write(std::stringstream & shader) const override
	{
		if (!m_glinfo.isGLES2 &&
			...
			(CombinerProgramBuilder::cycleType == G_CYC_COPY || CombinerProgramBuilder::textureConvert.useTextureFiltering()))
		{
			shader <<
...
public:
	...
	void write(std::stringstream & shader) const override
	{
		...
		if (!m_glinfo.isGLES2) {
			...
			if (CombinerProgramBuilder::textureConvert.useTextureFiltering()) {
			...
			if (CombinerProgramBuilder::textureConvert.useYUVCoversion()) {
				shaderPart +=
...
public:
	...
	void write(std::stringstream & shader) const override
	{
		...
		if (m_glinfo.isGLES2) {
			...
			if (CombinerProgramBuilder::textureConvert.getBilerp0()) {
...
public:
	...
	void write(std::stringstream & shader) const override
	{
		...
		if (m_glinfo.isGLES2) {
			...
			if (CombinerProgramBuilder::textureConvert.getBilerp1()) {
...
public:
	...
	void write(std::stringstream & shader) const override
	{
		...
		if (m_glinfo.isGLES2) {
			...
			if (config.video.multisampling > 0 && CombinerProgramBuilder::textureConvert.useTextureFiltering()) {
				shaderPart =
