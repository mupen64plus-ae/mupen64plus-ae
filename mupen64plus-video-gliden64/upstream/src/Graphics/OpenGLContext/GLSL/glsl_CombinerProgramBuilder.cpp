...
u32 CombinerProgramBuilder::cycleType = G_CYC_1CYCLE;
TextureConvert CombinerProgramBuilder::textureConvert;
...
static
bool needClampColor() {
	return CombinerProgramBuilder::cycleType <= G_CYC_2CYCLE;
}

static
bool combinedColorC(const gDPCombine & _combine) {
	if (CombinerProgramBuilder::cycleType != G_CYC_2CYCLE)
		return false;
...
CombinerInputs CombinerProgramBuilder::compileCombiner(const CombinerKey & _key, Combiner & _color, Combiner & _alpha, std::string & _strShader)
{
	...
	if (CombinerProgramBuilder::cycleType < G_CYC_FILL)
...
{
	CombinerProgramBuilder::cycleType = _key.getCycleType();
	CombinerProgramBuilder::textureConvert.setMode(_key.getBilerp());
	...
	if (bUseTextures) {
		...
		if (CombinerProgramBuilder::cycleType == G_CYC_2CYCLE && config.generalEmulation.enableLegacyBlending == 0)
		...
		else if (CombinerProgramBuilder::cycleType < G_CYC_COPY)
	...
	if (CombinerProgramBuilder::cycleType <= G_CYC_2CYCLE && m_useCoverage)
	...
	if (bUseTextures) {
		...
		if (bUseLod) {
			...
			if (CombinerProgramBuilder::cycleType < G_CYC_COPY) {
				if (combinerInputs.usesTile(0))
